...
const logger = LoggerUtil.getLogger('DOCK');

function dockingCommand(call, callback) {
  logger.info('New request /dockingCommand !');
  ...
  reply.setLeaseUseResult(leaseUseResult).setStatus(1).setDockingCommandId(123);

  callback(null, reply);
}

function dockingCommandFeedback(call, callback) {
  logger.info('New request /dockingCommandFeedback !');
  ...
  reply.setLeaseUseResult(leaseUseResult).setStatus(2);
